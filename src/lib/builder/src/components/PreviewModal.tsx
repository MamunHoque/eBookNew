...
const PreviewModal: React.FC<PreviewModalProps> = ({ isOpen, onClose, pages, currentPage, canvasSize }) => {
  ...
  useEffect(() => {
    if (isOpen && previewRef.current) {
      const containerWidth = previewRef.current.clientWidth - 80;
      const containerHeight = previewRef.current.clientHeight - 80;
      const pageAspectRatio = canvasSize.width / canvasSize.height;
      const containerAspectRatio = containerWidth / containerHeight;

      let newScale;
      if (containerAspectRatio > pageAspectRatio) {
        newScale = containerHeight / (canvasSize.height * 96);
      } else {
        newScale = containerWidth / (canvasSize.width * 96);
      }
  ...
  const currentPageData = pages.find(page => page.pageNumber === displayPage);
  const elements: Element[] = currentPageData ? JSON.parse(currentPageData.content) : [];
          ...
          <div ref={previewRef} className="flex-grow bg-gray-200 dark:bg-gray-900 relative" style={containerStyle}>
            <div style={pageStyle}>
              {elements.map((element: Element) => (
                  ...
                  ...element.style, // Apply any custom styles from the element
                }}
